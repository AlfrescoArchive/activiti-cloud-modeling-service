...
public class ProjectService {
  ...
  public Page<Project> getProjects(Pageable pageable,
    ...
    return projectRepository.getProjects(pageable, projectName);
  }
  ...
  private Optional<FileContent> createFileContentFromZipEntry(ZipStream.ZipStreamEntry zipEntry) {
    return zipEntry.getContent().map(
      bytes -> getContentTypeByPath(zipEntry.getFileName())
        .map(contentType -> new FileContent(zipEntry.getFileName(), contentType, bytes))).orElse(Optional.empty());
  }
